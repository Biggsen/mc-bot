...
ipcMain.handle("recorder:runIgloos", async (event, { projectId, inputDatasetId, connection }) => {
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "igloos", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
  ...
  const outputName = `Igloos with Y (${new Date().toLocaleString(undefined, {
    dateStyle: "short",
    timeStyle: "short",
  })})`;
  const index2 = await loadIndex();
...
ipcMain.handle("recorder:runSwampHuts", async (event, { projectId, inputDatasetId, connection }) => {
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "swamp_huts", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
  ...
  const outputName = `Swamp huts with Y (${new Date().toLocaleString(undefined, {
    dateStyle: "short",
    timeStyle: "short",
  })})`;
  const index2 = await loadIndex();
...
ipcMain.handle("recorder:runTrailRuins", async (event, { projectId, inputDatasetId, connection }) => {
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "trail_ruins", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
  ...
  const outputName = `Trail ruins with Y (${new Date().toLocaleString(undefined, {
    dateStyle: "short",
    timeStyle: "short",
  })})`;
  const index2 = await loadIndex();
...
ipcMain.handle("recorder:runOceanRuins", async (event, { projectId, inputDatasetId, connection }) => {
  if (activeRecorderRun) {
    throw new Error("A bot session or recorder run is already active");
  }
  const index = await loadIndex();
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "ocean_ruins", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
  ...
  const outputName = `Ocean ruins with Y (${new Date().toLocaleString(undefined, {
    dateStyle: "short",
    timeStyle: "short",
  })})`;
  const index2 = await loadIndex();
...
ipcMain.handle("recorder:runWoodlandMansions", async (event, { projectId, inputDatasetId, connection }) => {
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "woodland_mansions", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
  ...
  const outputName = `Woodland mansions with Y (${new Date().toLocaleString(undefined, {
    dateStyle: "short",
    timeStyle: "short",
  })})`;
  const index2 = await loadIndex();
...
ipcMain.handle("recorder:runBuriedTreasure", async (event, { projectId, inputDatasetId, connection }) => {
  ...
  const bot = createBot(botConfig);
  const controller = new AbortController();
  activeRecorderRun = { type: "buried_treasure", bot, controller };
  attachEvents(bot, botConfig, { onEnd: () => {} });
